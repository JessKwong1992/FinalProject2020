  ...
  function (
    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      setConfig: function (config) {
        ...
        var threatInfos = null;
        if (this.config.hasOwnProperty("lpgThreatTypes")) {
          lpgThreatData = this.config.lpgThreatTypes;
          this._lpgThreatTypeTable.clear();
          if (lpgThreatData.length > 0) {
            for (var i = 0; i < lpgThreatData.length; i++) {
              threatInfos = lpgThreatData[i];
              this._populateThreatTableRow(threatInfos, this._lpgThreatTypeTable, "fireBallDiameter", false);
              if (!unitType) {
        ...
        for (var j = 0; j < threatData.length; j++) {
          threatInfos = threatData[j];
