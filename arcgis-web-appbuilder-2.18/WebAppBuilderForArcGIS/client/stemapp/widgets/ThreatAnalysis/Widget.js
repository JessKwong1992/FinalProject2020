    ...
    return declare([BaseWidget, dijitWidgetBase, dijitWidgetsInTemplate], {
      ...
      _threatCreateButtonClicked: function () {
        if (!domClass.contains(this.threatCreateButton, 'jimu-state-disabled')) {
          var threatCatagory = this.threatCatagorySelect.get("value");
          this._createThreatAreas(this._currentGeometry, this._selectedThreat, this._selectedUnitType, threatCatagory);
        }
      },
        ...
        array.forEach(data, lang.hitch(this, function (threatInfo) {
          if (threatInfo.Unit === "meters") {
            if (isChemicalThreat) {
              ...
              threatInfo.Bldg_Dist = this._convertToFeet(threatInfo.Fireball_Dia);
              threatInfo.Outdoor_Dist = this._convertToFeet(threatInfo.Safe_Dist);
            }
          }
